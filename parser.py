        ...
        # Cadenas que contengan con '
            ...
            for strchar1 in xrange(len(strings1)):
                if (strings1[strchar1] == '\'' or strings1[strchar1] == '\"') and flag_open:
                    ...
                    lst_lexer.append([tokens[strings1[strchar1]], row, lc + 1])
                    lc += len(string1)+1
                    flag_open = False
                    break
            ...
            for char_i in xrange(len(chars)):
                if (chars[char_i].lower() in alpha_list) or (chars[char_i].lower() in numbers_list):
                    ...
                    if char in tokens:
                        ...
                        lst_lexer.append([tokens[(l[lc:lc + char_i]).lower()], row, lc + 1])
                    else:
                        # print '<'+'id'+','+ l[lc:lc + char_i] + ',' + str(row) + ',' + str(lc + 1) + '>'
                        lst_lexer.append(['id', row, lc + 1])
                    lc += len(char)
                    break
            ...
            for digit in xrange(len(digits)):
                if digits[digit] in numbers_list:
                    ...
                    if '.' in real_or_int:
                        ...
                        lst_lexer.append(['token_real', row, lc + 1])
                    else:
                        # print '<'+'token_entero'+',' + real_or_int + ',' + str(row) + ',' + str(lc + 1) + '>'
                        lst_lexer.append(['token_entero', row, lc + 1])
        ...
        # Cadenas que contengan con operadores
            ...
            if l[lc] == '<':
                if l[lc+1] == '-':
                    ...
                    lst_lexer.append([tokens[l[lc:lc+2]], row, lc + 1])
                    lc += len(l[lc])
                elif l[lc+1] == '>':
                    # print '<'+tokens[l[lc:lc+2]] + ',' + str(row) + ',' + str(lc + 1) + '>'
                    lst_lexer.append([tokens[l[lc:lc+2]], row, lc + 1])
                    ...
                    lst_lexer.append([tokens[l[lc]], row, lc+1])
                    lc += len(l[lc])-1
            elif l[lc] == '>':
                if l[lc+1] == '=':
                    # print '<'+tokens[l[lc:lc+2]] + ',' + str(row) + ',' + str(lc + 1) + '>'
